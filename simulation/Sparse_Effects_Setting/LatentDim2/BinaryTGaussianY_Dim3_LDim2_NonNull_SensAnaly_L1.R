...
latent_dim <- 2
mu_u_t <- read.csv('LatentDim2/mu_u_t_ise.csv') %>% as.matrix()
cov_u_t <- read.csv('LatentDim2/cov_u_t_ise.csv') %>% as.matrix()
eigen_cov <- eigen(cov_u_t)
cov_halfinv <- eigen_cov$vectors %*% diag(eigen_cov$values^{-1/2}) %*% t(eigen_cov$vectors)
u_t_diff_org_all <- read.csv('LatentDim2/u_t_diff_org_all.csv') %>% as.matrix()
...
summary(lmfit_confound) # Multiple R-squared:  0.5582,	Adjusted R-squared:  0.5582
...
gamma_int_min 
gamma_opted1
gamma_opted1 <- c(104.62134, -82.17413)
## R^2 ##
...
(auc_naive <- DescTools::AUC(x = fpr_naive, y = tpr_naive)) # 0.54
(auc_cali <- DescTools::AUC(x = fpr_cali, y = tpr_cali)) # 0.61
