...
scatter_est <- tibble(index = rep(1:100))
...
ggsave("LatentDim3/scatter_coef_MAD.pdf", plot = scatter_coef, width = 300, height = 190, units = "mm")
...
# MAD #
penalty_weights <-  seq(0, 12, by = 0.1)# c(seq(0.1, 0.9, by=0.005), 10)
tau_cali_mat <- matrix(0, nrow=length(penalty_weights), ncol=length(tau_t)) # 101*500
...
for(weight in penalty_weights) {
  ...
  results <- cal_tau_gamma(function (g) {
    ...
    mad(tau_cali) + weight * t(g) %*% cov_u_t %*% g / sigma_y_t^2 ## penalizing R^2
...
# MAD(tau_cali) VS R^2 #
plot(R2_vec, apply(tau_cali_mat, 1, mad), type = "l",
     ylab = expression("MAD("~tau~")"), xlab = expression(R[paste(Y,'~',U,'|',T)]^2))
plot(R2_vec, apply(tau_cali_mat[,- nontrivial_index], 1, mad), 
     ylab = expression("MAD("~tau~")"), xlab = expression(R[paste(Y,'~',U,'|',T)]^2))
points(R2_vec, apply(tau_cali_mat[,nontrivial_index], 1, mad), col = "blue")

## Scatter plot #
tibble(tau = as.numeric(tau_cali_mat[, c(trivial_seleted_index, nontrivial_index)]),
       R2 = rep(R2_vec, 100),
